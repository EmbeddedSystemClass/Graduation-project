...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
  ...
  u8 ar_rval2=0;
	...
	if(fftemp==NULL){rval=1;ar_rval1=1;}
	tempbuf=mymalloc(1024);	//分配4096个字节空间
	if(tempbuf==NULL){rval=1;ar_rval2=1;}
 	res=f_open(fftemp,(const TCHAR*)fxpath,FA_READ); 
 	if(res)rval=2;//打开文件失败 
	printf("ar_rval1=%d,ar_rval2=%d\n",ar_rval1,ar_rval2);
 	if(rval==0)	 
	{
		switch(fx)
		{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			SPI_Flash_Write(tempbuf,offx+flashaddr,1024);		//从0开始写入4096个数据  
	  		offx+=bread;	  
